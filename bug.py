...
class BugMine:
    ...
    def login(self):
        loginreq = urllib.request.Request(self.loginUrl, urllib.parse.urlencode(
            self.loginParams).encode('utf-8'), headers=BugMine.headers)
    ...
    def getMyBug(self):
        ...
        print(bugPage)
        myItems = re.findall(
            "<td\s+class='a-left\s+nobr'>[\s\b]*?<a\s+href='(.*?)'\s*>[\s\S]*?</a>[\s\b]*?</td>", bugPage, re.S)
        items = []
        for item in myItems:
            items.append(item)
bugMine = BugMine({
